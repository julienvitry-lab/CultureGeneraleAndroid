...
// Le navigateur Culture Générale n'invente ni question, ni détail, ni distracteur.
// Il affiche puis importe uniquement les QCM capturés par le serveur sur Quizypedia.
...
function validFullQuestionnaireUrl(raw){
  try{
    ...
    return /(^|\.)quizypedia\.fr$/i.test(u.hostname) && parts.length>=3 && norm(parts[0])==='quiz';
  }catch{return false;}
}
...
function sourceToDrafts(data){
  ...
  return (data.questions||[]).map(q=>({
    ...
    label:'Quizypedia 1:1',
    source_number:Number(q.source_number||0),
    answer_label:String(q.answer_label||'')
  })).filter(q=>
    q.question && q.options.length===4 &&
    q.options.every(Boolean) && q.correct_index>=1 && q.correct_index<=4
  );
}
...
function applyClassification(){
  ...
  render();
  status('Classification mégathème/thème appliquée. Aucun contenu Quizypedia n’a été modifié.','ok');
}
...
function render(){
  ...
  box.innerHTML=drafts.map((q,i)=>`
    <article class="cgimp5-card" data-i="${i}">
      <div class="cgimp5-head">
        <label class="cgimp5-check">
          <input type="checkbox" class="cgimp2-check" ${q.selected?'checked':''}>
          <span>Sélectionner</span>
        </label>
        <button type="button" class="cgimp5-edit-button">Modifier</button>
      </div>

      <div class="cgimp5-id">Quizypedia · ${esc(q.fiche||`Fiche ${q.source_number||''}`)}</div>
      <div class="cgimp5-theme">${esc(q.theme)}</div>

      <div class="cgimp5-question">${esc(q.question)}</div>
      ${q.detail?`<div class="cgimp5-clues">${esc(q.detail).replace(/\n/g,'<br>')}</div>`:''}

      <div class="cgimp5-options">
        ${q.options.map((v,j)=>`
          <div class="cgimp5-option ${j+1===q.correct_index?'cgimp5-correct':''}">
            <span class="cgimp5-letter">${'ABCD'[j]}.</span>
            <span>${esc(v)}</span>
          </div>`).join('')}
      </div>

      <div class="cgimp5-foot">
        <span>Réponse : ${q.correct_index}</span>
        <span>Source : Quizypedia 1:1</span>
        <span>Import : ${q.selected?'sélectionné':'non sélectionné'}</span>
      </div>

      <details class="cgimp5-edit-details">
        <summary>Modifier avant import</summary>
        <div class="cgimp2-edit">
          <label class="wide">Question<textarea data-field="question" rows="3">${esc(q.question)}</textarea></label>
          <label class="wide">Détail<textarea data-field="detail" rows="5">${esc(q.detail)}</textarea></label>
          ${q.options.map((v,j)=>`<label>${'ABCD'[j]}<input data-opt="${j}" value="${esc(v)}"></label>`).join('')}
          <label>Bonne réponse<select data-field="correct_index">${
            [1,2,3,4].map(n=>`<option value="${n}" ${n===q.correct_index?'selected':''}>${'ABCD'[n-1]}</option>`).join('')
          }</select></label>
        </div>
      </details>
    </article>
  ...
  box.querySelectorAll('.cgimp5-card').forEach(card=>{
    const i=Number(card.dataset.i),q=drafts[i];
    const details=card.querySelector('.cgimp5-edit-details');

    card.querySelector('.cgimp5-edit-button').addEventListener('click',()=>{
    ...
    card.querySelector('.cgimp2-check').addEventListener('change',e=>{
      ...
      renderCounters();
      const spans=card.querySelectorAll('.cgimp5-foot span');
      if(spans[2])spans[2].textContent=`Import : ${q.selected?'sélectionné':'non sélectionné'}`;
    });
...
async function analyze(){
  ...
  if(!validFullQuestionnaireUrl(url)){
    status('CGIMPORT008 exige l’URL complète : https://www.quizypedia.fr/quiz/<thème>/<questionnaire>/','err');
    return;
  }
  ...
  status('Capture stricte du questionnaire Quizypedia en cours… aucun contenu n’est généré.','warn');
  ...
  try{
    ...
    if(strictComplete){
      status(
        `✅ Capture 1:1 terminée : ${drafts.length} QCM capturé(s) sur ${data.fiches.length} fiche(s). `+
        `Question, détail et A/B/C/D proviennent de Quizypedia. Aucun import n’a encore eu lieu.`,
        'ok'
      );
    }else{
      const diag=(data.diagnostics||[]).slice(0,2).join(' · ');
      status(
        `⚠ Capture partielle : ${drafts.length}/${data.fiches.length}. Import désactivé pour respecter le mode strict 1:1.`+
...
async function importSelected(){
  if(!strictComplete){
    status('Import bloqué : la capture Quizypedia n’est pas complète en mode strict 1:1.','err');
    return;
  }
  const create=apiCreate();
  if(!create?.create){status('API de création Firestore indisponible.','err');return;}

  const selected=drafts.filter(q=>q.selected);
  if(!selected.length){status('Aucune question sélectionnée.','err');return;}

  if(!confirm(
    `Importer ${selected.length} QCM strictement capturé(s) depuis Quizypedia ?\n\n`+
    `Culture Générale attribuera uniquement ses propres ID Firestore.`
  ))return;
  ...
  try{
    ...
    status(
      ...
      (skipped?`, ${skipped} doublon(s) exact(s) question+détail ignoré(s)`:'')+
      '.',
      'ok'
    );
  }catch(e){
...
function relabelUi(){
  ...
  if(kicker)kicker.textContent='CGIMPORT008 · 1:1';
  if(sub)sub.textContent=
    'Capture un questionnaire Quizypedia sans générer ni reformuler son contenu : '+
    'question, détail et propositions sont repris de la source avant attribution d’un ID Culture Générale.';
