const CGWEB035_VERSION='CGWEB035_LEARNING_HUB001';
...
async function cg35Overview(){
  try{
    ...
    cg35SetBody(`<section class="cg35-kpis">
      ...
      <article><small>Difficulté</small><b>${cg35Fmt(s.averageDifficulty)}/100</b><span>difficulté observée personnelle</span></article>
    </section>
    ...
    <section class="cg35-domain-grid">${(d.domains||[]).map(g=>`<article class="cg35-card"><h3>${cg35Esc(g.name)}</h3><p>${cg35Fmt(g.questionCount)} vue(s) · ${cg35Fmt(g.successPercent)} % réussite</p><small>${cg35Fmt(g.priorityCount)} prioritaire(s) · ${cg35Fmt(g.dueCount)} due(s)</small></article>`).join('')||'<div class="cg35-empty">Aucune donnée de jeu.</div>'}</section>`);
...
async function cg35History(){
  try{
    ...
    cg35SetBody(`<div class="cg35-filterbar">${[['all','Tous'],['qcm','QCM'],['mental','Mental'],['revision','Révision']].map(([k,l])=>`<button data-history-filter="${k}" class="${CG35.historyFilter===k?'active':''}">${l}</button>`).join('')}</div>
      <div class="cg35-submeta">${cg35Fmt((d.rows||[]).length)} affiché(s) · 100 derniers maximum</div>
...
function cg35Metric(g,view){
  if(view==='mastery'){const m=g.mastery||{};return `${cg35Fmt(m['Maîtrisée']||0)} maîtrisée(s) · ${cg35Fmt(m['Fragile']||0)} fragile(s) · ${cg35Fmt(m['À réviser']||0)} à réviser`}
  if(view==='due')return `${cg35Fmt(g.dueCount)} révision(s) échue(s) · ${cg35Fmt(g.successPercent)} % réussite`;
  if(view==='weakness')return `Faiblesse ${cg35Fmt(g.weakness)}/100 · ${cg35Fmt(g.priorityCount)} prioritaire(s)`;
  if(view==='response')return `Médiane ${cg35Time(g.medianResponseMs)} · ${cg35Fmt(g.knownSlowCount)} connue(s) mais lente(s)`;
  if(view==='difficulty')return `Difficulté ${cg35Fmt(g.difficulty)}/100 · ${cg35Fmt(g.attempts)} réponse(s)`;
  return '';
}
async function cg35Groups(view,domain=''){
  try{
    ...
    cg35SetBody(`<div class="cg35-viewhead"><div><h3>${cg35Esc(title)}</h3><p>${domain?'Choisis un thème.':'Choisis un domaine.'}</p></div>${domain?'<button id="cg35BackGroups">← Domaines</button>':''}</div>
      <section class="cg35-list">${(d.rows||[]).map(g=>`<button class="cg35-group" data-group="${cg35Esc(g.name)}"><strong>${cg35Esc(g.name||'(Sans thème)')}</strong><span>${cg35Fmt(g.questionCount)} question(s) · ${cg35Fmt(g.successPercent)} % réussite</span><small>${cg35Esc(cg35Metric(g,view))}</small></button>`).join('')||'<div class="cg35-empty">Aucune donnée.</div>'}</section>`);
...
function cg35QuestionMetric(q,view){
  if(view==='mastery')return `${q.mastery} · ${q.successPercent}% réussite · ${q.attempts} réponse(s)`;
  ...
  if(view==='weakness')return `Faiblesse ${q.weakness}/100 · ${q.failures} erreur(s) · ${q.successPercent}% réussite`;
  ...
  if(view==='difficulty')return `${q.difficultyLabel} · ${q.difficulty}/100 · ${q.confidence}`;
  return '';
}
async function cg35Questions(view,domain,theme){
  try{
    ...
    cg35$('cg35BackThemes').onclick=()=>cg35Groups(view,domain);cg35$('cg35CopyIds').onclick=()=>cg35Copy(rows.map(x=>x.id));cg35Status(`✅ ${rows.length} question(s).`,'ok');
  }catch(e){cg35SetBody(`<div class="cg35-error">${cg35Esc(e.message)}</div>`);cg35Status(`❌ ${e.message}`,'bad')}
}
...
async function cg35NeverOverview(){
  try{
    ...
    cg35SetBody(`<div class="cg35-viewhead"><div><h3>Jamais vues</h3><p>${cg35Fmt(d.unseen)} jamais vue(s) sur ${cg35Fmt(d.total)} questions</p></div></div>
      <section class="cg35-list">${(d.rows||[]).map(r=>`<button class="cg35-group" data-never-domain="${cg35Esc(r.name)}"><strong>${cg35Esc(r.name)}</strong><span>${cg35Fmt(r.unseen)} jamais vue(s)</span><small>${cg35Fmt(r.seen)} vue(s) · ${cg35Fmt(r.total)} total</small></button>`).join('')}</section>`);
...
async function cg35NeverThemes(domain){
  try{
    ...
    cg35SetBody(`<div class="cg35-viewhead"><div><h3>Jamais vues · ${cg35Esc(domain)}</h3><p>Choisis un thème.</p></div><button id="cg35NeverBack">← Domaines</button></div>
      <section class="cg35-list">${(d.rows||[]).map(r=>`<button class="cg35-group" data-never-theme="${cg35Esc(r.name)}"><strong>${cg35Esc(r.name||'(Sans thème)')}</strong><span>${cg35Fmt(r.unseen)} jamais vue(s)</span><small>${cg35Fmt(r.seen)} vue(s) · ${cg35Fmt(r.total)} total</small></button>`).join('')||'<div class="cg35-empty">Aucun thème jamais vu.</div>'}</section>`);
...
async function cg35NeverQuestions(domain,theme){
  try{
    ...
    cg35$('cg35NeverThemeBack').onclick=()=>cg35NeverThemes(domain);cg35$('cg35CopyIds').onclick=()=>cg35Copy(rows.map(x=>x.id));cg35Status(`✅ ${rows.length} jamais vue(s) affichée(s).`,'ok');
  }catch(e){cg35SetBody(`<div class="cg35-error">${cg35Esc(e.message)}</div>`);cg35Status(`❌ ${e.message}`,'bad')}
}
function cg35Wire(){
...
function cg35Init(){
  ...
  const p=document.createElement('section');p.id='cgweb035Panel';p.className='cg35-panel';p.innerHTML=`<header class="cg35-head"><div><div class="cg35-kicker">CGWEB035 · LEARNING_HUB001</div><h2>Apprentissage</h2><p>Historique, maîtrise, révisions, points faibles, vitesse et difficulté.</p></div><button id="cg35Refresh">Actualiser</button></header>
