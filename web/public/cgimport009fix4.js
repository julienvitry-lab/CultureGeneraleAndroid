...
function ensureBatchUi(){
  ...
  if(!$('cgimp9Style')){
    ...
    style.textContent=`
      ...
      .cgimp9-missing{font-size:12px;font-weight:900;color:#ffd36b;margin-top:4px}
      ...
      ${questionnaires.map((q,i)=>`
        <div class="cgimp9-row" data-i="${i}">
          ...
          <div>
            ...
            ${q.missingLabel?`<div class="cgimp9-missing">Fiche manquante : ${esc(q.missingLabel)}</div>`:''}
...
async function captureQuestionnaireItem(item,index,total){
  ...
  try{
    ...
    item.missingLabel=item.missingFiches.length
      ? item.missingFiches
          .map(f=>`${String(f.name||'').trim()} — n°${Number(f.number||0)}/${Number(f.total||item.expected||0)}`)
          .join(' ; ')
      : '';
    item.diagnostic=(data.diagnostics||[]).slice(0,4).join(' · ');
...
async function captureDirect(url){
  ...
  try{
    ...
    if(strictComplete){
      ...
      const missing=(data.missingFiches||[])
        .map(f=>`${f.name} — n°${f.number}/${f.total||data.fiches.length}`)
        .join(' ; ');
      status(
        `⚠ Capture partielle : ${drafts.length}/${data.fiches.length}. Import désactivé.`+
        (missing?` Fiche manquante : ${missing}.`:'')+
