...
function rowHtml(r){const p=[r.proposition_a,r.proposition_b,r.proposition_c,r.proposition_d].filter(v=>v!==null&&v!==undefined&&String(v)!=="").map((v,i)=>`<span><b>${String.fromCharCode(65+i)}.</b> ${esc(v)}</span>`).join("");return `<article class="cg6-row"><div class="cg6-row-head"><div><div class="cg6-id">#${esc(r.original_id??r.id)}</div><div class="cg6-path">${esc(r.megatheme)}${r.theme?" › "+esc(r.theme):""}</div></div><button class="cg6-edit" data-edit="${esc(r.id)}">Modifier</button></div><div class="cg6-question">${esc(r.question)}</div>${r.detail?`<div class="cg6-detail">${esc(r.detail)}</div>`:""}${p?`<div class="cg6-props">${p}</div>`:""}<div class="cg6-meta"><span>Réponse : ${esc(r.correct_index??"—")}</span><span>Statut : ${esc(r.status??"—")}</span><span>row : ${esc(r.row_number??"—")}</span><span>Révision : ${esc(r.cg_revision??0)}</span></div></article>`}
