...
public class MainActivity extends Activity {
    ...
    private com.google.firebase.firestore.ListenerRegistration
            cgSync006TombstoneRegistration;

    private boolean cgSync006ContentCatchupRunning = false;
    private boolean cgSync006TombstoneCatchupRunning = false;

    private final android.os.Handler cgSync006Handler =
            new android.os.Handler(android.os.Looper.getMainLooper());

    private Runnable cgSync006ContentRetryRunnable;
    private Runnable cgSync006TombstoneRetryRunnable;
    // CGSYNC006_FIELDS_END
...
final String currentHash =
    ...
    private void showHome() {
        ...
        syncState.setOnClickListener(v -> showCgSync006Dashboard());
        setRoundedBackgroundWithStroke(syncState, DARK, 12, Color.WHITE, 1);
        LinearLayout.LayoutParams syncStateLp =
                new LinearLayout.LayoutParams(-1, cmToPx(0.9f));
        syncStateLp.setMargins(0, cmToPx(0.12f), 0, cmToPx(0.12f));
        root.addView(syncState, syncStateLp);
        // CGSYNC006_HOME_BUTTON_END
    ...
    private void addRevisionAssimilateButton(Question q) {
        ...
        assimilate.setEnabled(!isQuestionAssimilated(q.row));
    ...
    private boolean isQuestionAssimilated(long row) {
        SQLiteDatabase db = openDb();
        Cursor c = null;
        try {
            c = db.rawQuery(
            ...
            return c.moveToFirst() &&
                    "A".equalsIgnoreCase(safe(c.getString(0)).trim());
        } finally {
            if (c != null) c.close();
            db.close();
        }
    ...
    private void assimilateTrioQuestion(Question q, Button assimilate) {
        ...
        new Thread(() -> updateStatusForRow("A", q.row)).start();
    }
